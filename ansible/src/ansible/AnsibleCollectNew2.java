...
public class AnsibleCollectNew2 {
    ...
    private static Map<String, JSONObject> containerNetStatMap = new HashMap<String, JSONObject>();
    ...
    public JSONObject mergeNetStatProc(String netStatJsonStr, String target) {
        return mergeNetStatProc(netStatJsonStr, target, null);
    }

    public JSONObject mergeNetStatProc(String netStatJsonStr, String target, String containerIP) {
        JSONObject netStatObj;
        JSONObject ContainerInfo = getContainerInfo(target, true);
        JSONObject hostNetStatObj;
        JSONArray netStatArray = new JSONArray();
        if (netStatJsonStr == null) {
            netStatObj = new JSONObject();
        }
        else {
            netStatObj = new JSONObject(netStatJsonStr);
        }
        if (netStatObj.has(target)) {
            hostNetStatObj = netStatObj.getJSONObject(target);
            if (!"SUCCESS".equals(hostNetStatObj.getString("exitFlag"))) {
                return hostNetStatObj;
            }
            netStatArray = hostNetStatObj.getJSONArray("Conns");
        }
        else if (ContainerInfo != null && ContainerInfo.has(containerIP)) {
            netStatArray = ContainerInfo.getJSONObject(containerIP).getJSONArray("Netstat");
        }
        else {
        ...
        for (int i = 0; i < netStatArray.length(); i++) {
            ...
            if (containerIP != null) {
                oneConn.put("procName", ContainerInfo.getJSONObject(containerIP).getString("Name"));
            }
            else {
    ...
    public JSONObject mergeNetStatPort(String netStatJsonStr, String target) {
        return mergeNetStatPort(netStatJsonStr, target, null);
    }
     ...
     * @return
     */
    public JSONObject mergeNetStatPort(String netStatJsonStr, String target, String containerIP) {
        JSONObject netStatObj;
        JSONObject ContainerInfo = getContainerInfo(target, true);
        JSONObject hostNetStatObj;
        JSONArray netStatArray = new JSONArray();
        ...
        for (int i = 0; i < netStatArray.length(); i++) {
            ...
            for (int j = i; j < netStatArray.length(); j++) {
                ...
                if (remoteIP.equals(remoteConns.getString("ip"))) {
                    ...
                    if (containerIP != null) {
                        local_procName = ContainerInfo.getJSONObject(containerIP).getString("Name");
                    }
                    else {
    ...
    public JSONObject getProcInfoByPort(String target, String port) {
        ...
        // 可能是容器的内部端口，内部容器端口是不确定的(可能是非映射的容器内部端口,一个机器的不同容器中可能有相同port)
        error.put("output", "Failed to get process name.").put("procName", "unknown");
        return error;
    }

    public JSONObject getContainerNetstat(String target, String pid) {
        ...
        JSONObject netstat = new DockerNetstatNew().update(tcpStr, tcp6Str);
        // JSONArray established = netstat.getJSONArray("establish");
        containerNetStatMap.put(target + "/" + pid, netstat);
        return netstat;
    }

    public JSONObject getContainerInfo(String target, boolean isGetNetstat) {
        ...
        if (StringUtil.isNotEmpty(outputStr)) {
            JSONObject oneContainer, netstat, tempObj;
            int pid;
            ...
            for (int i = 0; i < outputJArray.length(); i++) {
                ...
                if (!tempObj.getJSONObject("State").has("Pid")) {
                    continue;
                }
                pid = tempObj.getJSONObject("State").getInt("Pid");
                ...
                if (isGetNetstat) {
                    netstat = getContainerNetstat(target, String.valueOf(pid));
                    oneContainer.put("Netstat", netstat.getJSONArray("netstat"));
                }
                if (IPAddress != null && !IPAddress.isEmpty()) {
                    jResult.put(IPAddress, oneContainer);
                }
                else if (netstat.has("container_ip")) {
                    jResult.put(netstat.getString("container_ip"), oneContainer);
                }
            }
        }
    ...
    public String getAllHostStat() {
        ...
        localPortPid.clear();
        String target[] = {
            "10.45.80.23", "10.45.80.24", "10.45.80.41", "10.45.80.42", "10.45.80.43", "10.45.80.44", "10.45.80.64"
        // "10.45.80.26", "10.45.80.23", "10.45.80.24"
        };
        JSONObject results = new JSONObject();
        NetstatRunnable threadArray[] = new NetstatRunnable[target.length];
        for (int i = 0; i < threadArray.length; i++) {
            threadArray[i] = new NetstatRunnable(target[i], 0);
            threadArray[i].start();
        }
        try {
            JSONArray containerResult;
            String cIP;
            JSONObject cNetstat;
            for (NetstatRunnable runnable : threadArray) {
                runnable.join();
                results.put(runnable.getTarget(), runnable.getResult());
                containerResult = runnable.getConatinerResult();
                for (int i = 0; i < containerResult.length(); i++) {
                    cIP = containerResult.getJSONObject(i).getString("c_ip");
                    cNetstat = containerResult.getJSONObject(i).getJSONObject("netstat");
                    results.put(cIP, cNetstat);
                }
            }
        }
    ...
    class NetstatRunnable extends Thread {
        ...
        private JSONArray containerResult;
        ...
        public JSONArray getConatinerResult() {
        ...
        public void run() {
            ...
            String ip;
            JSONObject containerNetstat;
            containerResult = new JSONArray();
            switch (flag) {
                case 0:
                    result = mergeNetStatPort(getNetstat(null, target), target);
                    while (iterator.hasNext()) {
                        ip = (String) iterator.next();
                        containerNetstat = new JSONObject();
                        containerNetstat.put("c_ip", ip);
                        containerNetstat.put("netstat", mergeNetStatPort(null, target, ip));
                        containerResult.put(containerNetstat);
                    }
                    break;
                case 1:
                    ...
                    while (iterator.hasNext()) {
                        ...
                        containerNetstat = new JSONObject();
                        containerNetstat.put("c_ip", ip);
                        containerNetstat.put("netstat", mergeNetStatProc(null, target, ip));
                        containerResult.put(containerNetstat);
                    }
                    break;
            }
    ...
    public static void main(String[] args) {
        ...
        // linux下测试
        // java -classpath ansible.jar:json-20140107.jar ansible.AnsibleCollectNew2
        if (args.length == 0) {
            String result = new AnsibleCollectNew2().getNetstat("10.45.80.64");
            System.out.println(new AnsibleCollectNew2().mergeNetStatPort(result, "10.45.80.26"));
        }
        else if (args.length == 1) {
            File f = new File("/var/output.txt");
            try {
                FileWriter fw = new FileWriter(f);
                fw.write(new AnsibleCollectNew2().getAllHostStat());
                fw.close();
            }
            catch (IOException e) {
                e.printStackTrace();
        ...
        // System.out.println(new AnsibleCollectNew2().getContainerNetstat("10.45.80.26", "1272").toString());
    }
}
